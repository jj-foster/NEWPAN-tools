...
def read_vel(vel_file:str):
    ...
    return x,y,z,u,v,w,Cp

def normalize_line_length(x:list,y:list,z:list) -> list:
    lengths=[]
    total_len=0
...
def plot_vel_1D(xs,ys,series,ax=None,vmin=None,vmax=None):
    # limit
    if vmin==None:
        vmin=min(ys)
    if vmax==None:
...
if __name__ == "__main__":
    ...
    x,y,z,u,v,w,Cp=read_vel(vel_file)
    xs=normalize_line_length(x,y,z)

    fig,ax=plt.subplots()

    plot_vel_1D(xs,u,"u",ax=ax)#,vmin=-0.5,vmax=2)
    plot_vel_1D(xs,v,"v",ax=ax)
    plot_vel_1D(xs,w,"w",ax=ax,vmin=-5,vmax=5)
